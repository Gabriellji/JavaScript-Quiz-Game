...
const returnCurrentQuiz = (array) => array[currentQuestionIndex];
...
const playMusic = (file) => {
    ...
    const startPlayPromise = music.play();
...
CHARACTERS_BUTTONS.forEach((button) => {
    ...
    button.addEventListener("click", (e) => {
        ...
        setTimeout(() => {
            PRE_GAME_WRAP.style.display = "none";
        }, 1000);
    });
});
...
ANSWER_BUTTONS.forEach((btn) => {
    btn.addEventListener("click", (el) => {
        ...
        setTimeout(() => {
            TIP_WRAP.classList.remove("show-tip");
        }, 2000);
    });
});
...
BUTTONS.forEach((elem) => {
    elem.addEventListener("click", (e) => {
        ...
        const optionToBoolean = e.target.innerHTML.toLowerCase() === "true";
        const answer = returnCurrentQuiz(quizQuestions).isCorrect;
        if (optionToBoolean == answer) {
            ...
            if ((currentQuestionIndex + 1) % 4 === 0) {
                ...
                SCORE_BUST_TEXT.addEventListener("click", () => {
                    ...
                    setTimeout(() => {
                        ONE_SECOND_DIV.classList.add("fade-out-bck");
                    }, 2000);
                    setTimeout(() => {
                        ONE_SECOND_DIV.classList.remove("fade-out-bck", "fade-in-fwd");
                    }, 3000);
                });
            }
